class CurrencyConverter extends React.Component {
  ...
  handleUsdChange(e) {
    const euro = this.convert(e.target.value, this.state.rate, this.toEuro);
    this.setState({
      usd: e.target.value,
      euro
    });
  ...
  handleEuroChange(e) {
    const usd = this.convert(e.target.value, this.state.rate, this.toUsd);
    this.setState({
      euro: e.target.value,
      usd
    });
